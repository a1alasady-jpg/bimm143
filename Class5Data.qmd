...
author: "Ahmed Alasady PID: A18518453"
format: pdf
toc: true
---
...
ggplot(cars) +
  ...
  labs (x="Speed (MPH)",
        y="Distance(FT)",
        title= "Stopping Distnce of old Cars") +
  theme_bw()
```
...
Here we explore the famous `gapmider` data set with some cutsom plots.
...
ggplot(gapminder) +
  ...
  geom_point() 
```
I want to see a plot for each continet - in ggplot lingo this is called "faceting".
...
```{r}
insert <- filter(gapminder, year == 2007 | year== 1977)
```
